...
namespace GRINS
{
    ...
    : Physics(physics_name, input)
  {
    ...
    for(; _enabled_subdomains_it !=  _enabled_subdomains.end() ;++_enabled_subdomains_it)
      {
	std::cout<<"Reduced Electrosomosis enable on subdomain: "<<*_enabled_subdomains_it;
      }
  ...
  void ReducedElectroosmosis::init_context( AssemblyContext& context )
  {
...
{ 
  ...
  DenseSubMatrix<Number> &KuT = context.get_elem_jacobian(_u_var, _T_var);
  DenseSubMatrix<Number> &Kuu = context.get_elem_jacobian(_u_var, _u_var);
  
  DenseSubMatrix<Number> &KvT = context.get_elem_jacobian(_v_var, _T_var);
	...
	case 1 : // Station 1
	  {
	    ...
	    //std::cout<<"Setting boundary condition for station 1 using the reduced physics."<<std::endl;
