...
namespace GRINS
{
  ...
  {
    this->_Debye_Huckel = input("Physics/FullElectroosmosis/lambda", 1.0); 
  ...
  void FullElectroosmosis::init_context( libMesh::FEMContext& context )
  {
    // We should prerequest all the data
    // we will need to build the linear system
    // or evaluate a quantity of interest.
    context.element_fe_var[_E_var]->get_JxW();
    context.element_fe_var[_E_var]->get_phi();
    context.element_fe_var[_E_var]->get_dphi();    

    context.side_fe_var[_E_var]->get_JxW();
    context.side_fe_var[_E_var]->get_phi();
    context.side_fe_var[_E_var]->get_dphi();    
						...
						CachedValues& /*cache*/ )
  {
    ...
    //libmesh_assert (n_u_dofs == context.dof_indices_var[_E_var].size());    
    ...
    libMesh::DenseSubMatrix<Number> &KEE = *context.elem_subjacobians[_E_var][_E_var]; // R_{E},{E}
    ...
    libMesh::DenseSubVector<Number> &FE = *context.elem_subresiduals[_E_var]; // R_{E}
...
{ 
  ...
  // The subvectors and submatrices we need to fill:
  DenseSubMatrix<Number> &KEE = *context.elem_subjacobians[_E_var][_E_var];
  ...
  DenseSubVector<Number> &FE = *context.elem_subresiduals[_E_var];
