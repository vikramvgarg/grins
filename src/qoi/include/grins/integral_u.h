...
#ifndef GRINS_INTEGRAL_U_H
#define GRINS_INTEGRAL_U_H
...
namespace GRINS
{
  //! Integral_U QoI
  /*!
    This class implement a Integral_U QoI that can be used to both compute
    ...
    problem.
   */
  class Integral_U : public QoIBase
  {
  public:
    ...
    /*! Constructor takes GetPot object to read any input options associated
        with this QoI */
    Integral_U( const std::string& qoi_name );
    ...
    /*! Currently, only implemented for 2D. Assumes that the Integral_U will be
        computed over area of input subdomain id. Integral_U computed as 
  ...
  inline
  bool Integral_U::assemble_on_interior() const
  {
    return true;
  }

  inline
  bool Integral_U::assemble_on_sides() const
  {
    return false;
  }
}
#endif //GRINS_INTEGRAL_U_H
